...
lr_cb = ReduceLROnPlateau(
    monitor="val_f1", factor=0.5, patience=20, min_lr=1e-6, mode="max", verbose=1
)
...
def build_quadrant_model(input_shape, output_dim):
    ...
    # filterSizes = [8, 16, 32, 64, 128]  # 128, 128]  # leave out 128, 128
    filterSizes = [16, 16, 32, 64]  # 128, 128]  # leave out 128, 128
...
def load_dataset(image_dir, classes, cells, regions, grid, output_size=None,mode="region"):
    ...
    def generator():
        for label_file in label_files:
            ...
            bboxes = np.array(
                data["bboxes"], dtype=np.float32
            )  # read as float32 for further processing
...
def split_dataset(ds, train_size, val_size, test_size):
    ...
    val_ds = indexed_ds.filter(
        lambda i, _: (i >= train_size) & (i < train_size + val_size)
    ).map(lambda _, x: x)
    test_ds = indexed_ds.filter(lambda i, _: i >= train_size + val_size).map(
        lambda _, x: x
    )
...
def focal_loss(gamma=2.0, alpha=0.25):
    def loss(y_true, y_pred):
        ...
        weight = alpha * y_true * tf.pow(1 - y_pred, gamma) + (1 - alpha) * (
            1 - y_true
        ) * tf.pow(y_pred, gamma)
